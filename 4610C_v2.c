...
int liftChange(int old, int degs) // Lift Up using encoders
{
	if(degs < old) // Lift UP
	{
		if(nMotorEncoder[liftRight] > degs && !resetButton)
		{
			motor[liftLeft]  = -127;
			motor[liftRight]  = -127;
			return old;
	...
	{
		if(nMotorEncoder[liftRight] < abs(degs) && !resetButton)
		{
			motor[liftRight]  = 127;
			motor[liftLeft]  = 127;
			return old;
...
int elevatorChange(int old, int degs) // Lift Up using encoders
{
	...
	else if(old > degs) // Lift DOWN
	{
		if(SensorValue[elevatorPot] < abs(degs) && !resetButton)
		{
			motor[elevator]  = -127;
			return old;
		}
...
int clawChange(int old, int degs) // Lift Up using encoders
{
	...
	else if (degs  > old) // Claw Close
	{
		if(nMotorEncoder[elevator] < abs(degs) && !resetButton)
		{
			motor[claw]  = -127;
			return old;
		}
...
void watchdog()
{
	oldlift = liftChange(oldlift, liftGoal);
	if(oldlift == liftGoal)
		liftDone = 1;
