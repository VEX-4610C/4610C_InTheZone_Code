...
#pragma config(Motor,  port6,           liftLeft,      tmotorVex393_MC29, openLoop, encoderPort, I2C_3)
#pragma config(Motor,  port7,           liftRight,     tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_2)
...
int liftDone = 0;
int curStacked = 0;
int stackLevels[6] = {650, 600, 565, 540, 525, 520};
...
void degmove(int distance)
{
	...
	{
		while(abs(nMotorEncoder[backLeft]) < abs(distance))
		{
			motor[frontLeft] = motor[backLeft] = -127;
			motor[frontRight] = motor[backRight] = -127;
		}
...
float liftKP = 0.25, liftKI = 0, liftKD = 0.07;
long liftTotalError;
...
{
	writeDebugStreamLine("%d %d", time10[T1], stopElevator);
...
task autonomous()
{
	...
	*/
	if(tester == 0)
	{
		...
		wait1Msec(5000);
		motor[backRight] = motor[frontRight] = -60;
		motor[backLeft] = motor[frontLeft] = -60;
